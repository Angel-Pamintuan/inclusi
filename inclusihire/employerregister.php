...
if (!isset($_SESSION['usertype']) || $_SESSION['usertype'] !== 'Employer') {
    $_SESSION['status'] = "Access denied. Please select Employer first.";
    header("Location: whoyouare.php");
    exit();
}

// Set `user_type_id` for Employer (assuming Employer is 2 in DB)
$usertype_id = 2;
...
if (isset($_POST['submit'])) {
    ...
    $company_name = $_POST['company_name'];  // Get company_name from the form
    $industry = $_POST['industry'];  // Get industry from the form

    // Debug: Check values of company_name and industry
    ...
    // Insert into tbl_user (set fullname and phone as NULL)
    $insert_query = "INSERT INTO tbl_user (email, password, usertype_id) 
                     VALUES (?, ?, ?)";
    $stmt = $conn->prepare($insert_query);
    $stmt->bind_param("ssi", $email, $hashedPassword, $usertype_id);

    if ($stmt->execute()) {
        // Get the last inserted user ID
        $new_user_id = $stmt->insert_id;

        // Debug: Check if we get a new_user_id
        echo "New User ID: " . $new_user_id . "<br>";

        // Insert the provided company_name and industry into tbl_employer_profile
        $insert_profile = "INSERT INTO tbl_employer_profile (user_id, company_name, industry, company_type)
                   VALUES (?, ?, ?, ?)";
        $stmt_profile = $conn->prepare($insert_profile);

        // Assuming the company_type_id of 1 exists in tbl_company_type
        $company_type_id = 1;  // Or set this to a valid value based on your logic

        $stmt_profile->bind_param("issi", $new_user_id, $company_name, $industry, $company_type_id);

        // Debug: Check if insert query is executing
        if ($stmt_profile->execute()) {
            // Redirect to employer home page after successful registration
            $_SESSION['user_id'] = $new_user_id;
            header("Location: employerhome.php");
            exit();
        } else {
            $_SESSION['status'] = "Error occurred while creating employer profile.";
            header("Location: employerregister.php");
            exit();
        }
    ...
    <div class="container-fluid">
        ...
        <form method="POST" action="">
            <div class="form-floating mb-3">
                <input type="text" name="company_name" class="form-control custom-input" id="floatingInput"
            ...
            <div class="form-floating mb-3">
                <input type="text" name="industry" class="form-control custom-input" id="floatingInput"
                    placeholder="Any" required>
                <label for="floatingInput" class="custom-label">
                    <ion-icon name="business-outline"></ion-icon> Industry
                </label>
            </div>
        ...
        <div class="login">Have an account? <a href="employerlogin.php" style="text-decoration: none;"
